...
public class FlatSVGUtils {
    ...
    /**
     * Creates from the given SVG a list of icon images with different sizes
     * that can be used for windows headers. The SVG should have a size of
     * 16x16, otherwise it is scaled.
     * <p>
     * If running on Windows in Java 9 or later and multi-resolution image
     * support is available, then a single multi-resolution image is returned
     * that creates images on demand for requested sizes from SVG. This has the
     * advantage that only images for used sizes are created. Also if unusual
     ...
     * {@code module-info.java}. Otherwise use
     * {@link #createWindowIconImages(URL)}.
     *
     * @param svgName the name of the SVG resource (a '/'-separated path)
     * @return list of icon images with different sizes (16x16, 20x20, 24x24,
     * 28x28, 32x32, 48x48 and 64x64)
     ...
     * the SVG is not opened in {@code module-info.java}. E.g.
     * {@code createWindowIconImages( getClass().getResource( "/com/myapp/myicon.svg" ) )}.
     *
     ...
     * This method is useful if using Java modules and the package containing
     * the SVG is not opened in {@code module-info.java}. E.g.
     * {@code svg2image( getClass().getResource( "/com/myapp/myicon.svg" ), 24, 24 )}.
     *
     * @param svgUrl the URL of the SVG resource
     * @param width the width of the image
     * @param height the height of the image
     ...
     * <p>
     * If using Java modules, the package containing the SVG must be opened in
     * {@code module-info.java}. Otherwise use {@link #svg2image(URL, float)}.
     *
     * @param svgName the name of the SVG resource (a '/'-separated path)
     ...
     * This method is useful if using Java modules and the package containing
     * the SVG is not opened in {@code module-info.java}. E.g.
     * {@code svg2image( getClass().getResource( "/com/myapp/myicon.svg" ), 1.5f )}.
     *
     * @param svgUrl the URL of the SVG resource
     * @param scaleFactor the amount by which the SVG size is scaled
     * @return the image
