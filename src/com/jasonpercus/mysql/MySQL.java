...
//METHODES PUBLICS
    ...
    public void connect() throws java.sql.SQLException {
        if(autoreconnect)
            this.conn = (com.mysql.jdbc.Connection) java.sql.DriverManager.getConnection("jdbc:mysql://"+ip+":"+port+"/"+base+"?autoReconnect=true&useUnicode=yes", username, password);
        else{
            if(useunicode)
                this.conn = (com.mysql.jdbc.Connection) java.sql.DriverManager.getConnection("jdbc:mysql://"+ip+":"+port+"/"+base+"?useUnicode=yes", username, password);
            else
                this.conn = (com.mysql.jdbc.Connection) java.sql.DriverManager.getConnection("jdbc:mysql://"+ip+":"+port+"/"+base, username, password);
        }
    }
