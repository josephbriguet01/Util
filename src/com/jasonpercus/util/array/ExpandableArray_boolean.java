...
//CONSTRUCTORS
    /**
     * Crée un tableau extensible de boolean
     */
    public ExpandableArray_boolean() {
        super();
    }
    
    /**
     * Crée un tableau extensible de boolean avec pour valeurs de départ les booleans du tableau array
     * @param array Correspond au tableau qui fourni les valeurs de départ
     */
    @SuppressWarnings("OverridableMethodCallInConstructor")
    public ExpandableArray_boolean(boolean... array){
        super();
        if(array != null)
            this.put(array);
    }
     ...
     */
    @SuppressWarnings("OverridableMethodCallInConstructor")
    public ExpandableArray_boolean(ExpandableArray_boolean array){
        super();
        if(array != null)
            this.put(array);
    }
...
//METHODES PUBLICS
    /**
     * Renvoie un boolean à l'index donné
     * @param index Correspond à l'index du boolean à récupérer
    ...
    public synchronized boolean get(int index) {
        ...
        boolean[] array = (boolean[]) listArrays.get(div);
        return array[mod];
    }

    /**
     * Place un boolean dans le tableau. Si le tableau est trop petit, celui-ci est étendu automatiquement
     * @param index Correspond à l'index où placer le boolean
     * @param b Correspond au boolean à placer
     */
    public synchronized void set(int index, boolean b) {
        if (index < 0) {
            throw new java.lang.ArrayIndexOutOfBoundsException("" + index);
        }
    ...
    /**
     * Place un ou plusieurs boolean à la suite du dernier boolean placé
     * @param array Correspond au tableau de booleans à placer à la suite
     */
    public synchronized void put(boolean... array){
        for(boolean b : array){
            set(size, b);
        }
    }
    ...
    /**
     * Ajoute un tableau extensible de booleans à la suite du dernier boolean placé
     * @param array Correspond au tableau extensible de booleans à placer à la suite
     */
    public synchronized void put(ExpandableArray_boolean array){
        for(boolean b : array){
            put(b);
        }
    }
    ...
    /**
     * Renvoie l'index d'un boolean dans le tableau
     * @param b Correspond au boolean à trouver
     * @return Retourne l'index du boolean trouvé dans le tableau ou -1 si le boolean n'a pas été trouvé
     */
    public synchronized int indexOf(boolean b){
    ...
    /**
     * Renvoie le dernier index d'un boolean dans le tableau
     * @param b Correspond au boolean à trouver
     * @return Retourne le dernier index du boolean trouvé dans le tableau ou -1 si le boolean n'a pas été trouvé
     */
    public synchronized int lastIndexOf(boolean b){
    ...
    /**
     * Renvoie si oui ou non un boolean existe dans le tableau
     * @param b Correspond au boolean à trouver
     * @return Retourne true si le boolean existe, sinon false
     */
    public synchronized boolean contains(boolean b){
        return indexOf(b) > -1;
    }
    
    /**
     * Efface une cellule du tableau. Le tableau conserve sa taille actuelle, mais la valeur de la cellule passe à false
    ...
    /**
     * Efface le tableau. Le tableau conserve sa taille actuelle, mais toute les cellules du tableau passent à false
     */
    ...
    /**
     * Efface toutes les cellules du tableau contenant le boolean en paramètre. Le tableau conserve sa taille actuelle, mais la valeur des cellules passent à false
     * @param b Correspond au boolean à supprimer
     */
    public synchronized void removeAll(boolean b){
        int index = indexOf(b);
        while (index > -1)
            remove(index);
    }
    ...
    public synchronized boolean[] toArray(){
        ...
        for(int i=0;i<countArray;i++){
            boolean[] toAdd = (boolean[]) listArrays.get(i);
            if(i+1 == countArray){
     ...
     */
    public synchronized boolean objEquals(ExpandableArray_boolean other){
     ...
     */
    public synchronized void arraycopy(ExpandableArray_boolean src, int srcPos, int destPos, int length){
        for(int i=srcPos;i<(srcPos+length);i++){
            boolean element = src.get(i);
            set(destPos++, element);
        }
    }
     ...
     * @param length Correspond au nombre d'objet à copier dans le tableau courant
     */
    public synchronized void arraycopy(boolean[] src, int srcPos, int destPos, int length){
        for(int i=srcPos;i<(srcPos+length);i++){
            boolean element = src[i];
            set(destPos++, element);
        }
    }
    ...
    public java.util.Iterator<Boolean> iterator() {
        ExpandableArrayIterator_boolean<Boolean> iterator = new ExpandableArrayIterator_boolean<>(this);
        return iterator;
    }
...
//METHODES PRIVATES
     ...
     */
    private synchronized boolean[] lastArray() {
        if (listArrays.isEmpty()) {
            return null;
        } else {
            return (boolean[]) listArrays.get(listArrays.size() - 1);
        }
    }

    /**
     * Ajoute un boolean dans un sous tableau
     * @param array Correspond au sous tableau
     * @param index Correspond à l'index du sous tableau
     * @param b Correspond au boolean à ajouter
     */
    private synchronized void addE(boolean[] array, int index, boolean b) {
        array[index] = b;
    }
    ...
    private synchronized void addA(int count) {
        for (int i = 0; i < count; i++) {
            listArrays.add(new boolean[sizeSubArray]);
            countArray++;
        }
     ...
     * @return Retourne un tableau de taille fixe
     */
    private synchronized Boolean[] toArrayBoolean(){
        Boolean[] out = new Boolean[size];
        if(size == 0) return out;
        int added = 0;
        for(int i=0;i<countArray;i++){
 ...
 * @see ExpandableArray_boolean
 * @author JasonPercus
 * @param <E> Correspond au type d'objet contenu dans le tableau extensible
 */
class ExpandableArrayIterator_boolean<E> implements java.util.Iterator<E> {
...
//ATTRIBUTS
     ...
     */
    private final ExpandableArray_boolean array;
...
//CONSTRUCTOR
     ...
     */
    protected ExpandableArrayIterator_boolean(ExpandableArray_boolean array) {
        this.array      = array;
        this.position   = 0;
    }
